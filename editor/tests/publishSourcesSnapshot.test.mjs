...
describe('отпечаток источников в записи — тот, что соответствует созданному дереву', () => {
  it('late.png появилась во время update-ref служебной ссылки — отправки нет, push=0, очередь и ссылка прибраны', async () => {
    ...
    // Новый план доступен, и человек не заперт. Картинку, на которую не ссылается ни один язык,
    // он с собой не берёт: она лишняя (SPEC 5.4.3), и следующая запись уберёт её в корзину.
    const снова = await состав(место);
    expect(снова.status).toBe(200);
    expect(снова.payload.файлы.map((файл) => файл.путь)).not.toContain(`${ПАПКА}/late.png`);
  });
...
`);
  ...
  it('файл появился во время построения плана — записи нет: коммит описывал бы не сегодняшний диск', async () => {
    const место = await среда();
    // Картинка названа в тексте: иначе она была бы лишней (SPEC 5.4.3), и правило судило бы об
    // уборке, а не об отпечатке источников.
    место.положить(RU, `${СТАТЬЯ}![Схема](./late.png)\n`);
