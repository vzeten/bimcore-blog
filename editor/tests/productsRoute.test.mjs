...
const SETTINGS = {
  ...
  ошибкиСервера: {
    ...
    картинкаТовараНеДоехала: 'картинку товара скачать не удалось: {причина}',
    картинкаТовараОтказХранилища: 'хранилище не отдало файл (ответ {подробность})',
    картинкаТовараПеревес: 'файл больше {подробность} МБ',
    картинкаТовараНетСвязи: 'нет связи ({подробность})',
  },
};
...
describe('каталог товаров и картинка товара', () => {
  ...
  it('обрыв скачивания не оставляет ни файла в статье, ни следа в карантине', async () => {
    ...
    expect(ответ.data.error).toBe('картинку товара скачать не удалось: хранилище не отдало файл (ответ 404)');
    expect(файлы(repo)).toEqual(['index.mdx']);
    expect(карантин(repo)).toEqual([]);
  });
