...
describe('поля шапки в человеческом виде', () => {
  ...
  it('очищенное описание уходит из шапки строкой целиком: пустых ключей не бывает', () => {
    // Прежнее правило писало сюда `description: ""` — оно отменено разбором шапки 2026-08-10.
    // Голое `description:` YAML читает как `null`, и сборка падает на «must be a string»;
    // `description: ""` сборку проходит, но даёт страницу вовсе без мета-описания, тогда как
    // отсутствие ключа отдаёт под него первый абзац статьи (проверено кодом Docusaurus 3.10).
    const шапка = 'description: "старое"';
    const поля = readFields(шапка, DOCS, КОРНИ).map((f) => ({...f, display: ''}));
    const собрано = writeFields(шапка, поля, DOCS, КОРНИ);

    expect(собрано).toBe('');
    expect(прочитать(собрано).description).toBe(undefined);
  });
