...
describe('красные буквы локали следуют опубликованной версии, а не местной', () => {
  const версия = (правки: Record<string, unknown>) => ({
    опубликован: true, отличается: false, правкаВЧерновике: false, заглушка: false,
    скрыта: false, скрытаВВетке: false, черновикСайта: false, черновикВВетке: false, ...правки,
  });

  it('красный цвет даёт «недоступно» в ОПУБЛИКОВАННОЙ версии и ничто другое', () => {
    expect(цветЛокали(признакиЛокали(версия({черновикВВетке: true})))).toBe('черновик');
  ...
  it('поле показывает МЕСТНОЕ состояние, даже когда на сайте опубликовано другое', () => {
    // Иначе владельцу нечем готовить следующую публикацию: он выбрал бы режим и не увидел выбора.
    expect(режим('title: A\ndraft: true')).toBe(НЕДОСТУПНО);
    expect(режим('title: A\nunlisted: true')).toBe(ПО_ССЫЛКЕ);
    expect(режим('title: A')).toBe(ДОСТУПНО);
    // А цвет букв в это же время отвечает про опубликованную версию, и они не спорят.
    expect(цветЛокали(признакиЛокали(версия({отличается: true, черновикСайта: true, черновикВВетке: true}))))
      .toBe('черновик');
  });

  it('выбранное в окне «Недоступно» до публикации букв не красит', () => {
    // Поле показывает местное состояние, чтобы его можно было готовить; цвет обещает сайт.
    const п = признакиЛокали(версия({отличается: true, черновикСайта: true}));
