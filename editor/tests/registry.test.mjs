...
describe('статья как одна сущность', () => {
  ...
  it('раздел статьи-файла — папки над самим файлом, а не над его папкой', () => {
    // Второй читатель правила адреса: раздел строится тем же `folderChain`. У файла не в папке
    // прежнее правило теряло `lessons` и уводило статью в корень документации.
    expect(sectionOf('docs/lessons/a.mdx', НАСТРОЙКИ)).toEqual({kind: 'docs', path: 'lessons'});
  });

  it('статья-файл в корне раздела остаётся в корне: живой реестр правкой адреса не сдвинут', () => {
    // Так лежит «Добро пожаловать» (`docs/intro.mdx`) — единственный файл-статья живого сайта.
