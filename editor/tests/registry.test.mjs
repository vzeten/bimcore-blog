...
describe('статья как одна сущность', () => {
  ...
  it('раздел записи вне канона — папки над самим файлом, а не над его папкой', () => {
    // Второй читатель правила адреса: раздел строится тем же `folderChain`. Так лежат три корневые
    // страницы документации (`docs/intro.mdx` и переводы) — единственные записи вне канона.
    expect(sectionOf('docs/lessons/a.mdx', НАСТРОЙКИ)).toEqual({kind: 'docs', path: 'lessons'});
    expect(sectionOf('docs/intro.mdx', НАСТРОЙКИ)).toEqual({kind: 'docs', path: ''});
    expect(sectionOf(`${RU}/intro.mdx`, НАСТРОЙКИ)).toEqual({kind: 'docs', path: ''});
  });

  it('index-файл раздела с вложенными статьями — служебная страница со своей категорией', () => {
    // Категорию реестра по-прежнему даёт вложенная статья, а не `_category_.json`: вид записи и
    // категория реестра — намеренно разные понятия, и требования к шапке от вида не зависят.
    const статьи = groupArticles(
      [
        файл('docs/guides/families/index.mdx', 'Revit Families', {категорияРядом: true}),
...
describe('реестр статей', () => {
  ...
  it('сортировка по просмотрам: числа приходят отдельно, статьи без числа уходят вниз', () => {
    const [первая, вторая, третья] = статьи;
    const просмотры = {[первая.key]: 5, [вторая.key]: 900};
    const по = sortArticles([первая, вторая, третья], 'просмотры', 'вниз', НАСТРОЙКИ, просмотры);
    expect(по.map((статья) => статья.key)).toEqual([вторая.key, первая.key, третья.key]);
  });
