...
import {DeleteAsk, чтоУйдёт} from '../src/ui/zones/DeleteAsk';
...
describe('разметка окон вопроса', () => {
  ...
  it('удаление: заголовок — название по основному языку, версии названы словами, ссылки — с числом в нужном падеже', () => {
    ...
    expect(html).toContain('Русская, английская и испанская версии уйдут в корзину и пропадут с сайта. Ссылки уберутся в 21 статье.');
  });

  it('удаление: одна версия — единственным числом; статьи нет на сайте — только корзина', () => {
    expect(чтоУйдёт(['ru'], true, НАСТРОЙКИ)).toBe('Русская версия уйдёт в корзину и пропадёт с сайта.');
    expect(чтоУйдёт(['en', 'ru'], false, НАСТРОЙКИ)).toBe('Русская и английская версии уйдут в корзину.');
    expect(чтоУйдёт(['es'], false, НАСТРОЙКИ)).toBe('Испанская версия уйдёт в корзину.');
  });
