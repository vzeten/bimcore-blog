 ...
 * или нет. Ручка спрашивает сайт так же, как публикация: `fetch` и `rev-parse FETCH_HEAD`.
 *
 ...
 * «сервер не ответил»: так проверяется случай, когда про сайт не известно ничего.
 */
export function gitСВеткой(опубликованные = []) {
  return {
    raw: async (аргументы) => {
      const args = Array.isArray(аргументы) ? аргументы : [];
      if (args[0] === 'fetch' || args.includes('FETCH_HEAD')) {
        if (опубликованные === null) throw new Error('сервер сайта не ответил');
        return args[0] === 'fetch' ? '' : '0000000000000000000000000000000000000001';
      }
      if (!args.includes('ls-tree')) return 'Проверка';
      const свои = args.slice(args.indexOf('--') + 1);
