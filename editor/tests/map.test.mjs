...
/** Весь код программы по определению SPEC 4.9, кроме тестов: их перечень — вывод `editor:test`. */
...
describe('карта модулей', () => {
  const файлы = [...кодовые(path.join(EDITOR, 'src')), path.join(EDITOR, 'server.mjs')].map(
  ...
  it('карта модулей не указывает на несуществующие файлы', () => {
    const упомянутые = [...карта.matchAll(/`([^`\n]+\.(?:mjs|ts|tsx|css))`/g)]
      .map((m) => m[1])
      .filter((p) => p === 'server.mjs' || p.startsWith('src/'));
