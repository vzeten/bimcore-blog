...
describe('знаки разметки открываются активным абзацем, одинаково у жирного и курсива', () => {
  ...
  it('жёсткий перенос двумя пробелами абзац не делит: знаки открываются по обе стороны', () => {
    // Два пробела в конце строки сайт рисует переносом ВНУТРИ абзаца, значит и пара знаков цела.
    const текст = 'Первый **жирный** тут.  \nВторой **жирный** тут.\n\nДругой абзац **тут**.\n';
    const from = текст.indexOf('Первый');
    expect(видно(текст, {from, to: from}))
      .toBe('Первый **жирный** тут.  \nВторой **жирный** тут.\n\nДругой абзац тут.\n');
  });

  it('жёсткий перенос косой абзац не делит тем же правилом', () => {
    const текст = 'Первый **жирный** тут.\\\n Второй *курсив* тут.\n';
    const from = текст.indexOf('Второй');
    expect(видно(текст, {from, to: from})).toBe(текст);
  });

  it('выделение, кончающееся в начале соседнего абзаца, его знаков не открывает', () => {
    const текст = 'Первый **жирный** тут.\n\nВторой **жирный** тут.\n';
    const второй = текст.indexOf('Второй');
    // Выделен ровно первый абзац с пустой строкой: последняя ЗАТРОНУТАЯ позиция — ещё не второй.
    expect(видно(текст, {from: 0, to: второй}))
      .toBe('Первый **жирный** тут.\n\nВторой жирный тут.\n');
  });

  it('выделение, зашедшее в соседний абзац хотя бы на букву, открывает знаки обоих', () => {
    const текст = 'Первый **жирный** тут.\n\nВторой **жирный** тут.\n';
    const второй = текст.indexOf('Второй');
    expect(видно(текст, {from: 0, to: второй + 1})).toBe(текст);
  });
...
describe('слой правки курсива красит фразу целиком тем же правилом, что и жирный', () => {
  const КУРСИВ = settings.вставки.find((item) => item.подпись === 'Курсив')!;
  /** Нажатие кнопки «Курсив»: новый текст файла. */
  const курсивом = (text: string, at: Selection): string => {
    const edit = decideEdit(КУРСИВ, text, at, {settings: {} as never, articlePath: ''}) as Edit;
    return `${text.slice(0, edit.from)}${edit.insert}${text.slice(edit.to)}`;
  };

  const текст = курсивом(ФАЙЛ, СЛОВО);
  const правки = [{fromA: СЛОВО.from, toA: СЛОВО.to, fromB: СЛОВО.from, toB: СЛОВО.to + 2}];

  it('изменение только курсива целиком относится к слою «Сейчас»', () => {
    expect(текст).toBe('Параметр *Ушки по бокам* включаются к любому изголовью.\n');
    const слои: Layer[] = [{text: ФАЙЛ, kind: 'site'}, {text: текст, kind: 'current', правки}];
    expect(слой(слои, 'Ушки по бокам')).toBe('current');
    expect(слой(слои, 'Параметр')).toBe('site');
  ...
  it('в смешанном старом и уже изменённом тексте новый курсив красит словосочетание целиком', () => {
    const сайт = 'Параметр Ушки по бокам включаются.\n';
    const файл = 'Параметр Ушки по краям включаются.\n';
    const окно = 'Параметр *Ушки по краям* включаются.\n';
    const слои: Layer[] = [{text: сайт, kind: 'site'}, {text: файл, kind: 'prevHuman'},
      {text: окно, kind: 'current', правки: [{fromA: 9, toA: 22, fromB: 9, toB: 24}]}];

    expect(слой(слои, 'Ушки по')).toBe('current');
    expect(слой(слои, 'краям')).toBe('current');
  });

  it('после сохранения и открытия курсив остаётся правкой того, кто его поставил', () => {
    const слои: Layer[] = [{text: ФАЙЛ, kind: 'site'}, {text: текст, kind: 'prevHuman'}];
    expect(слой(слои, 'Ушки по бокам')).toBe('prevHuman');
    expect(слой(слои, 'Параметр')).toBe('site');
  });

  it('существующий курсив прошлого слоя сам собой не перекрашивается в новую правку', () => {
    const слои: Layer[] = [{text: текст, kind: 'site'}, {text: текст, kind: 'prevAi'},
      {text: текст, kind: 'current'}];
    expect(слой(слои, 'Ушки по бокам')).toBe('site');
  });

  it('половину чужой пары `**` курсив за свою не принимает: жирное красится своим правилом', () => {
    const жирно = 'Параметр **Ушки** и **бокам** тут.\n';
    const слои: Layer[] = [{text: 'Параметр Ушки и бокам тут.\n', kind: 'site'},
      {text: жирно, kind: 'current'}];
    // Между двумя жирными кусками лежит старое слово: окраска на него не растекается.
    expect(слой(слои, ' и ')).toBe('site');
    expect(слой(слои, 'Ушки')).toBe('current');
    expect(слой(слои, 'бокам')).toBe('current');
  });

  it('незакрытая пара курсива не утягивает окраску на остальной текст', () => {
    const начало = 'Раз два.\nТри четыре.\n';
    const окно = 'Раз *два.\nТри четыре.\n';
    const слои: Layer[] = [{text: начало, kind: 'site'}, {text: окно, kind: 'current'}];
    expect(слой(слои, 'Три четыре')).toBe('site');
