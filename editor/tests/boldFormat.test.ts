...
describe('кнопка «Жирный» пишет в файл ровно `**выделенное**`', () => {
  ...
  it('повторное применение к тому же куску оставляет markdown разбираемым и жирным', () => {
    const {текст} = нажать(нажать(ФАЙЛ, СЛОВО).текст, {from: 11, to: 24});
    expect(текст.replace(/\*+/g, '')).toBe(ФАЙЛ);
    expect(жирныеКуски(текст).some((кусок) => текст.slice(кусок.from, кусок.to).includes('Ушки по бокам'))).toBe(true);
  });
