...
describe('красные буквы локали означают записанный в её файле draft: true', () => {
  /** Поставить `draft: true` в шапку названной версии — так же, как это сделало бы сохранение. */
  const пометить = (repo, rel) => {
    const текст = fs.readFileSync(path.join(repo, rel), 'utf8');
    положить(repo, rel, текст.replace('title: "Проба"', 'title: "Проба"\ndraft: true'));
  };

  it('помечена одна версия — красная только она, соседние языки своего цвета не меняют', async () => {
    const с = await среда();
    пометить(с.repo, ПУТИ.en);

    const признаки = признакиРеестра(await свод(с));

    expect(признаки.en.черновик).toBe(true);
    expect(признаки.ru.черновик).toBe(false);
    expect(признаки.es.черновик).toBe(false);
  }, ЖДАТЬ_GIT);

  it('шапка и реестр берут этот признак из одного источника и показывают одно и то же', async () => {
    const с = await среда();
    пометить(с.repo, ПУТИ.en);
    ...
    expect(articleFacts(статьи, ПУТИ.ru, НАСТРОЙКИ).признакиЛокалей).toEqual(признакиРеестра(статьи));
    expect(articleFacts(статьи, ПУТИ.en, НАСТРОЙКИ).признакиЛокалей.en.черновик).toBe(true);
  }, ЖДАТЬ_GIT);

  it('без строки draft красноты нет ни у одной версии, даже у неуехавшей правки', async () => {
    // Красный цвет обещает одно: сайт эту страницу не соберёт. Неуехавшая правка — другой знак.
    const с = await среда();
    await автосохранить(с, ПУТИ.en, 'English text, edited.');
