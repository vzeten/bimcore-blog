...
describe('разные старые черновики одной версии', () => {
  ...
  /** Общий черновик статьи стал другим — как после любого следующего сеанса работы. */
  const ушёлВперёд = (с, тело) => saveDraft(с.корень, НАСТРОЙКИ, newDraft({
    path: REL, frontmatterRaw: 'title: Кровати', body: тело, отпечатокБазы: 'ОТП', когда: '2026-09-09T09:00:00.000Z',
  }));
  const убрать = (с, ключ) => fs.rmSync(path.join(с.корень, 'editor', НАСТРОЙКИ['хранение'][ключ]), {recursive: true, force: true});

  it('разобранный спор не заводится заново при следующем запуске', () => {
    const с = спорнаяСреда();
    перенос(с);
    убрать(с, 'папкаСпоров'); // Человек разобрал спор так, как ему предложено: убрал варианты.

    expect(перенос(с)['споров']).toBe(0);
    expect(споры(с).size).toBe(0);
    // Запись черновика снова открыта — ради этого разбор и затевался.
    ушёлВперёд(с, 'после разбора');
    expect(loadDraft(с.корень, НАСТРОЙКИ, REL)['body']).toBe('после разбора');
  });

  it('уже перенесённый текст не становится спором, когда общий черновик ушёл вперёд', () => {
    const с = среда();
    старыйЧерновик(с.код, REL, 'работа из копии кода');
    перенос(с);
    ушёлВперёд(с, 'работа следующего дня');

    expect(перенос(с)['споров']).toBe(0);
    expect(споры(с).size).toBe(0);
    expect(loadDraft(с.корень, НАСТРОЙКИ, REL)['body']).toBe('работа следующего дня');
  });

  it('память о предложенном пропала — работа предлагается снова, а не теряется', () => {
    const с = среда();
    старыйЧерновик(с.код, REL, 'работа из копии кода');
    перенос(с);
    убрать(с, 'файлПереноса');
    ушёлВперёд(с, 'работа следующего дня');

    expect(перенос(с)['споров']).toBe(1);
  });
