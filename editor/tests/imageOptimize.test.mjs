...
describe('конвертер картинок', {timeout: 60000}, () => {
  ...
  it('прозрачная картинка с плавным цветным фоном сохраняется PNG без потерь, а не палитрой с зерном', async () => {
    // Проба владельца 2026-09-16: палитра на размытом цветном фоне давала видимое зерно и полосы.
    const итог = await обработать(await фото({альфа: 0}));
    expect(итог.итог).toBe('заменить');
    expect(итог.род).toBe('png');
    // Байт строения кадра в IHDR: 3 — палитра, 6 — полный цвет с прозрачностью.
    expect(итог.байты[25]).not.toBe(3);
    // Без потерь: те же точки, что у исходника (800×600 уже уже страницы — размер не меняется).
    const точкиИсходника = await sharp(await фото({альфа: 0})).ensureAlpha().raw().toBuffer();
    expect(await sharp(итог.байты).ensureAlpha().raw().toBuffer()).toEqual(точкиИсходника);
    // Повтор ничего не меняет.
    expect((await обработать(итог.байты)).итог).toBe('оставить');
  });

  it('снимок экрана по-прежнему получает палитру: на плоских цветах она неотличима от исходника', async () => {
    const итог = await обработать(await плоская());
    expect(итог.род).toBe('png');
    expect(итог.байты[25]).toBe(3);
  });
