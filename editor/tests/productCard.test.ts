...
/**
 * Что человек видит в окне: текст статьи, из которого вырезано всё, что слой заменил своим видом.
 * Карточка встаёт своим знаком, любой другой блок — своим: они занимают место блока, а не своих
 * строк; скрытое служебное не оставляет вместо себя ничего. По этому тексту видно, сколько пустых
 * рядов осталось на экране.
 */
function видимыйТекст(текст: string): string {
  const слой = blockLayer(БЛОКИ, () => АДРЕС_RU);
  const состояние = EditorState.create({doc: текст, extensions: [слой]});
  const вОкне = состояние.doc.toString();
  const куски: {от: number; до: number; знак: string}[] = [];
  состояние.field(слой).between(0, вОкне.length, (от, до, значение) => {
    const заменено = вОкне.slice(от, до);
    const блок = заменено.startsWith('<ProductCard') ? 'КАРТОЧКА' : 'БЛОК';
    куски.push({от, до, знак: значение.spec.widget === undefined ? '' : блок});
  });

  let видимо = '';
  let место = 0;
  for (const кусок of куски.sort((один, другой) => один.от - другой.от)) {
    видимо += вОкне.slice(место, кусок.от) + кусок.знак;
    место = кусок.до;
  }

  return видимо + вОкне.slice(место);
}
...
describe('скрытая строка импорта не занимает места', () => {
  it('над карточкой остаётся один пустой ряд, как и без служебной строки', () => {
    const видно = видимыйТекст(ТЕКСТ_RU.split('\r\n').join('\n'));

    expect(видно).not.toContain('import ');
    expect(видно).toContain('\n\nКАРТОЧКА');
    expect(видно).not.toContain('\n\n\nКАРТОЧКА');
  });

  it('второй пустой ряд принадлежит человеку и остаётся на месте', () => {
    const текст = ["import productPreview from './product.png';", '', '', '<ProductCard', '  name="Двери"', '  image={productPreview}', '/>'].join('\n');

    expect(видимыйТекст(текст)).toBe('\nКАРТОЧКА');
  });
});
