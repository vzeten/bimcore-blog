...
describe('карта хранит только исключения из обычного текста', () => {
  ...
  it('пустая рядом только с пустыми, границей совета или краем документа — редактируемый пустой абзац', () => {
    ...
    // В самом конце статьи под закрытием — место для текста, а не разделитель: иначе курсору некуда уйти.
    expect(карта(':::tip\nТ\n:::\n').filter((о) => о.вид === 'разделитель')).toEqual([]);
  });
...
describe('ручной список из строк после Shift+Enter: косая переноса уходит вместе с набором маркера', () => {
  /** Набор текста в позиции — как печатает человек: та же транзакция и тот же вид события. */
  const набрать = (state: EditorState, где: number, что: string) =>
    state.update({changes: {from: где, insert: что}, selection: {anchor: где + что.length}, userEvent: 'input.type'});

  it('сверху вниз и снизу вверх: три строки через Shift+Enter становятся тремя пунктами без слешей', () => {
    const текст = 'Первая\\\nВторая\\\nТретья';
    let s = состояние(текст, 0).update({changes: {from: 0, insert: '- '}, selection: {anchor: 2}, userEvent: 'input.type'}).state;
    s = набрать(s, s.doc.toString().indexOf('Вторая'), '- ').state;
    s = набрать(s, s.doc.toString().indexOf('Третья'), '- ').state;
    expect(s.doc.toString()).toBe('- Первая\n- Вторая\n- Третья');

    let н = состояние(текст, 0);
    н = набрать(н, текст.indexOf('Третья'), '1. ').state;
    expect(н.doc.toString()).toBe('Первая\\\nВторая\n1. Третья');
    н = набрать(н, н.doc.toString().indexOf('Вторая'), '1. ').state;
    н = набрать(н, 0, '1. ').state;
    expect(н.doc.toString()).toBe('1. Первая\n1. Вторая\n1. Третья');
  });

  it('маркер набирается по знаку: после «-» перенос ещё жив, после пробела косая уходит той же транзакцией', () => {
    const s = состояние('Первая\\\nВторая', 0);
    const дефис = набрать(s, 8, '-');
    expect(дефис.newDoc.toString()).toBe('Первая\\\n-Вторая');
    const пробел = набрать(дефис.state, 9, ' ');
    expect(пробел.newDoc.toString()).toBe('Первая\n- Вторая');
    expect(пробел.newSelection.main.head).toBe('Первая\n- '.length);
    // Обе правки в одной транзакции: отмена вернёт и пробел, и косую.
    let правок = 0;
    пробел.changes.iterChangedRanges(() => { правок += 1; });
    expect(правок).toBe(2);
  });

  it('смешение с Enter, экранированная косая и код нетронуты', () => {
    const с = состояние('Первая\\\nВторая\n\nТретья', 0);
    expect(набрать(с, 8, '- ').newDoc.toString()).toBe('Первая\n- Вторая\n\nТретья');
    const э = состояние('Первая\\\\\nВторая', 0);
    expect(набрать(э, 9, '- ').newDoc.toString()).toBe('Первая\\\\\n- Вторая');
    const к = состояние('```\nПервая\\\nВторая\n```', 0);
    expect(набрать(к, 12, '- ').newDoc.toString()).toBe('```\nПервая\\\n- Вторая\n```');
  });

  it('другие начала блоков под переносом и вставка из буфера косую не трогают', () => {
    const текст = 'Первая\\\nВторая';
    for (const начало of ['> ', '## ', '<', '| ', '```']) {
      expect(набрать(состояние(текст, 0), 8, начало).newDoc.toString()).toBe(`Первая\\\n${начало}Вторая`);
    }
    const вставка = состояние(текст, 0).update({changes: {from: 8, insert: '- '}, selection: {anchor: 10}, userEvent: 'input.paste'});
    expect(вставка.newDoc.toString()).toBe('Первая\\\n- Вторая');
    // Маркер, набранный не под переносом, соседнюю косую не трогает.
    expect(набрать(состояние('Первая\\\nВторая\nТретья', 0), 15, '- ').newDoc.toString()).toBe('Первая\\\nВторая\n- Третья');
  });

  it('открытие без правки и перенос внутри абзаца ничего не меняют', () => {
    const текст = 'Первая\\\nВторая';
    expect(состояние(текст).doc.toString()).toBe(текст);
    expect(набрать(состояние(текст, 0), текст.length, 'X').newDoc.toString()).toBe('Первая\\\nВтораяX');
  });
});
