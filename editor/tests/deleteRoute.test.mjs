...
const НАСТРОЙКИ = {
  хранение: {файлСостояния: '_state.json', папкаЧерновиков: '.drafts', папкаСнимков: '.history', снимковНаВерсию: 5, папкаКорзины: '.trash', корзинаДней: 30},
  контент: [
...
describe('режим по настоящей ветке', () => {
  ...
  it('статья не в ветке и не в её истории — навсегда после подтверждения; уходят все версии, папки, черновик, история', async () => {
    const repo = репозиторий();
    saveDraft(path.join(repo, 'editor'), НАСТРОЙКИ, {path: RU, body: 'черновик', frontmatterRaw: '', отпечатокБазы: 'x'});
    saveSnapshot(path.join(repo, 'editor'), НАСТРОЙКИ, RU, 'снимок', 'Автор', '2026-09-01T10:00:00.000Z');
