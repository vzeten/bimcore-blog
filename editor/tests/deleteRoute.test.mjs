...
const НАСТРОЙКИ = {
  хранение: {файлСостояния: '_state.json', папкаЧерновиков: '.drafts', папкаСпоров: '.drafts-споры', папкаСнимков: '.history', папкаСведения: '.сведение', снимковНаВерсию: 5, папкаКорзины: '.trash'},
  контент: [
...
describe('режим один — корзина', () => {
  it('без подтверждения сервер только называет, что будет, и ничего не трогает', async () => {
    ...
    // Название — как в реестре, по основному языку: вопрос удаления называет статью, а не вкладку.
    expect(ответ.data).toEqual({режим: 'корзина', пути: [EN, RU], языки: ['en', 'ru'], название: 'Проба'});
