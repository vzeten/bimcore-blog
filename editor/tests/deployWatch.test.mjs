...
import {
  запомнитьВыкладку, итогЗапусков, итогРабот, опросить, отметитьПоказ, прочитатьВыкладку, записатьВыкладку, слежение,
...
describe('опрос', () => {
  ...
  it('25 минут без итога — перед отказом один последний вопрос; итога нет — «выкладку не видно», опрос кончается', async () => {
    const {fetch, вопросы} = сеть(ответ(200, {workflow_runs: []}));
  ...
  it('срок вышел, но итог давно готов (сервер перезапускали) — последний вопрос его и называет', async () => {
    const {fetch} = сеть(ответ(200, ОТВЕТЫ['успех']));
    const давно = new Date(Date.now() - (ОПРОС['пределМинут'] + 60) * 60_000).toISOString();

    const итог = await опросить({запись: запись({когда: давно}), fetch, опрос: ОПРОС});

    expect(итог).toEqual({запись: expect.objectContaining({состояние: 'обновлён'}), ждатьМс: null});
  });

  it('вопрос к GitHub всегда один: «разбудить» во время идущего вопроса второго не заводит', async () => {
    const где = fs.mkdtempSync(path.join(os.tmpdir(), 'editor-deploy-'));
    папки.push(где);
    const настройки = {...НАСТРОЙКИ, выкладка: {...НАСТРОЙКИ['выкладка'], опрос: {...ОПРОС, первыйСек: 0, идётСек: 3600, ждётСек: 3600}}};
    записатьВыкладку(где, настройки, запись({когда: new Date().toISOString()}));
    let вопросов = 0;
    let ответить = null;
    const fetch = () => {
      вопросов += 1;
      return new Promise((готово) => { ответить = готово; });
    };
    const пауза = (мс) => new Promise((готово) => setTimeout(готово, мс));

    const {разбудить} = слежение({editorDir: где, настройки: () => настройки, fetch});
    разбудить();
    await пауза(30);
    for (let раз = 0; раз < 5; раз += 1) {
      разбудить();
      await пауза(10);
    }
    expect(вопросов).toBe(1);

    const идёт = {workflow_runs: [{...ОТВЕТЫ['успех'].workflow_runs[0], status: 'in_progress', conclusion: null}]};
    ответить(ответ(200, идёт));
    await пауза(30);
    разбудить();
    await пауза(30);
    // Следующий вопрос — только через паузу идущего запуска, не сразу.
    expect(вопросов).toBe(1);
    expect(прочитатьВыкладку(где, настройки).состояние).toBe('идёт');
  });
});
