...
describe('уборка ссылок', () => {
  ...
  it('снята только английская страница: голый адрес в русском тексте ведёт на русскую — не наш и не непонятный; полный адрес — наш', () => {
    const цели = {адреса: new Set(['/blog/post']), файлы: new Set(['blog/post/index.mdx'])};
    const тело = 'Своя [версия](/blog/post/) и голый /blog/post словами.\n';
    expect(безСсылок(`${шапка}${тело}`, цели, откуда(путь, шапка))).toMatchObject({текст: `${шапка}${тело}`, заменено: 0, непонятные: []});
    expect(безСсылок(`${шапка}https://learn.bimcore.one/blog/post/\n`, цели, откуда(путь, шапка)).непонятные).toEqual(['/blog/post']);
  });
});
