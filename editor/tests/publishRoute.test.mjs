...
describe('ручка записи статьи', () => {
  ...
  it('картинки статьи, включая вложенную папку, уезжают вместе с ней', async () => {
    ...
    // Картинка названа в тексте: безымянный файл публикация считает лишним (SPEC 5.4.3); обложка лишней не бывает.
    место.положить(RU, `${СТАТЬЯ}Новая строка.\n\n![Схема](./img/один.jpg)\n`);
...
describe('пропавшие файлы уходят и с сайта', () => {
  ...
  it('картинку переименовали — на сайте остаётся только новое имя', async () => {
    ...
    // Новое имя названо в тексте: иначе картинка не переименована, а брошена (SPEC 5.4.3).
    место.положить(RU, `${СТАТЬЯ}![Схема](./обложка.png)\n`);
