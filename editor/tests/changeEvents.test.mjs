...
const строки = (база) => база.prepare('SELECT коммит, путь, вид, прежний_путь, адрес, прежний_адрес, автор, добавлено, убрано, картинок_добавлено, картинок_убрано, описание FROM события ORDER BY дата, коммит, путь').all();
...
describe('события из серверной main', {timeout: 120_000}, () => {
  ...
  it('правки сотрудника видны владельцу; слияние один раз; код мимо, картинка — внутри правки статьи; след публикации не двинут', async () => {
    ...
    // Картинка рядом с единственной статьёй папки — часть той же правки: одна публикация статьи — одно действие.
    ...
    expect(правка).toMatchObject({адрес: '/lessons/a-new', прежний_адрес: '/lessons/a', добавлено: 2, убрано: 0, картинок_добавлено: 1, картинок_убрано: 0, описание: null});
...
describe('изображения статей', {timeout: 120_000}, () => {
  it('картинки — внутри действия своей статьи: одно на статью в коммите; img/ рядом; ссылка решает между статьями папки; ничьи — никому', async () => {
    ...
    const база = await открыть(владелец);
    await разобратьНовые({git, база, вершина: await серверныйКоммит(git), сайт, предел: 100, пределДельтыБайт: 100_000, пометкаОбреза: '…'});
    // Статьи заготовки мира (a.mdx, blog/old) к случаю не относятся.
    const события = строки(база).filter((с) => /^(blog\/post|docs\/guides)\//.test(с.путь))
      .map((с) => [с.вид, с.путь, с.картинок_добавлено, с.картинок_убрано]).sort();
    // Первый коммит: картинки пришли с новыми статьями — они внутри «Новая» (g1/g2 называют свою картинку,
    // index.mdx — единственная статья папки post). Второй: текст не менялся — «правка» только с картинками:
    // у post три картинки одним действием; g2.png — только статье, что её называет; ничья картинка, служебный
    // файл и картинка вне статей — никому.
    // Обе публикации — в одну секунду, порядок по дате не определён: сравнение без порядка.
    expect(события).toEqual([
      ['добавление', 'blog/post/index.mdx', 2, 0],
      ['добавление', 'docs/guides/g1.mdx', 1, 0],
      ['добавление', 'docs/guides/g2.mdx', 1, 0],
      ['правка', 'blog/post/index.mdx', 2, 2],
      ['правка', 'docs/guides/g2.mdx', 1, 1],
    ]);
    const правка = база.prepare('SELECT адрес, дельта, добавлено, убрано, картинки FROM события WHERE вид = ? AND путь = ?').get('правка', 'blog/post/index.mdx');
    expect(правка).toMatchObject({адрес: '/blog/post', дельта: '', добавлено: 0, убрано: 0,
      картинки: '-img-01.jpg\n+img-01.jpg\n-img-02.jpg\n+img-03.jpg\n'});
    база.close();
  });
