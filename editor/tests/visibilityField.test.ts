...
// и правда о том, что видит сайт. Всё это — про ОТКРЫТУЮ языковую версию: видимость принадлежит
// версии, а не статье целиком (SPEC 2.8). Отдельной ручки видимости в программе нет.
...
describe('изменение ещё не на сайте', () => {
  const статья = (правки: Record<string, unknown>) => ({
    скрытаНаСайте: false, веткаИзвестна: true, внеСайта: false, ...правки,
  });

  it('видимость в окне совпала с веткой по открытой версии — про сайт сказать нечего', () => {
    expect(изменениеНеНаСайте(статья({скрытаНаСайте: true}), true)).toBe(false);
  });

  it('открытая версия расходится с сайтом — статус загорается', () => {
    expect(изменениеНеНаСайте(статья({скрытаНаСайте: false}), true)).toBe(true);
  });

  it('соседняя версия расходится с сайтом, а открытая совпадает — окно молчит', () => {
    // Видимость принадлежит версии (SPEC 2.8), и про соседнюю в ответе нет ничего вовсе:
    // сказать про неё нечего, пока её не открыли. Названная плата этого правила.
    expect(изменениеНеНаСайте(статья({скрытаНаСайте: true}), true)).toBe(false);
  });
  ...
  it('версии нет в опубликованной ветке — это тоже «ещё не на сайте», а не «в меню»', () => {
    ...
    expect(изменениеНеНаСайте(статья({скрытаНаСайте: null}), false)).toBe(true);
  });
  ...
  it('окно молчит про сайт там, где сайт проверить не удалось', () => {
    ...
    // «не знаю» не значит «нет». Порядок проверок важен: чтение ветки отвечает `null` и на сбой.
    expect(изменениеНеНаСайте(статья({веткаИзвестна: false, скрытаНаСайте: null}), true)).toBe(false);
  });

  it('признака нет вовсе — это поломка договора с сервером, а не факт про сайт: окно молчит', () => {
    expect(изменениеНеНаСайте(статья({скрытаНаСайте: undefined}), true)).toBe(false);
  });

  it('статья вне сайта про сайт не говорит вовсе: песочница живёт отдельно', () => {
    expect(изменениеНеНаСайте(статья({внеСайта: true, скрытаНаСайте: null}), true)).toBe(false);
  });
...
// Правило состава статьи. Боевого вызова у него сейчас нет: сохранение соседние версии
// не трогает. Оно остаётся сознательно — ближайшие читатели названы в `PLAN.md` и `MAP.md`:
// проверка совпадения адресов языковых версий (круг 2) и состав коммита публикации (круг 3).
