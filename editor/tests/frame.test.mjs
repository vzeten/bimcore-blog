...
describe('каркас окна', () => {
  ...
  it('экран аналитики из левой полосы выводится в корень страницы, а не внутрь полосы', () => {
    // Внутри полосы её оформление кнопок (квадрат 30×30) ложилось на все кнопки экрана (проба 2026-09-24).
    const полоса = fs.readFileSync(path.join(UI, 'zones/Rail.tsx'), 'utf8');
    expect(полоса).toMatch(/createPortal\(\s*<AnalyticsScreen[\s\S]*?document\.body/);
    expect(полоса.match(/<AnalyticsScreen/g)).toHaveLength(1);
  });
...
describe('настройки не врут', () => {
  ...
  const разделы = ['подписи', 'реестр', 'хранение', 'картинки', 'сервер', 'видимость', 'ошибкиСервера', 'выпуск', 'просмотры', 'аналитика', 'аналитикаОкно'];
...
describe('цвета окна заданы', () => {
  /**
   * Каждая `var(--x)` без запасного значения в стилях окна либо задана где-то в css, либо её ставит
   * код (`--layer-*` — цвета слоёв из настроек, `layerColors.ts`). Незаданная переменная молча
   * даёт пустоту: так окна вопросов остались без фона и слились с затемнением (проба 2026-09-19).
   */
  it('нет ни одной незаданной переменной цвета', () => {
    const стили = files(UI).filter((файл) => файл.endsWith('.css')).map((файл) => fs.readFileSync(файл, 'utf8'));
    const заданы = new Set(стили.flatMap((текст) => [...текст.matchAll(/(--[\w-]+)\s*:/g)].map((м) => м[1])));
    const задаётКод = (имя) => имя.startsWith('--layer-');
    const незаданные = стили.flatMap((текст) => [...текст.matchAll(/var\((--[\w-]+)\s*\)/g)].map((м) => м[1]))
      .filter((имя) => !заданы.has(имя) && !задаётКод(имя));

    expect([...new Set(незаданные)]).toEqual([]);
  });
});
