...
describe('технические подписи медиа', () => {
  ...
  it('кодовые ограды не считаются медиа: картинка, импорт, JSX и ролик внутри ``` и ~~~ не находятся и не правятся', () => {
    const ролик = "import v from './a.webm';\n\n<video controls muted loop playsInline style={{width: '100%', height: 'auto'}}>\n  <source src={v} type=\"video/webm\" />\n</video>";
    const текст = [
      ШАПКА.trimEnd(), '',
      '```md', '![](./в-ограде.jpg)', "import p from './в-ограде.png';", '<YouTube id="jjJDJRnYrTU" />', '<ProductCard name="К" />', '```', '',
      '~~~', ролик, '~~~~', '',
      '![](./снаружи.jpg)', '',
      '````', '![](./длинная.jpg)', '```', 'ещё внутри: ![](./внутри.jpg)', '````', '',
      '```', '![](./незакрытая.jpg)', '',
    ].join('\n');
    expect(ссылкиМедиа(текст).map((с) => с.адрес)).toEqual(['./снаружи.jpg']);
    expect(именаМедиа(текст)).toEqual(['снаружи.jpg', 'cover.jpg']);
    const итог = применить(текст);
    expect(итог).toBe(текст.replace('![](./снаружи.jpg)', '![Розетки: изображение 1](./снаружи.jpg)'));
    expect(анализ(текст).отчёт.изменено).toEqual([{что: './снаружи.jpg', было: '', стало: 'Розетки: изображение 1'}]);
    expect(заменитьАдреса(текст, new Map([['в-ограде.jpg', 'x.png'], ['снаружи.jpg', 'y.png']]))).toBe(текст.replace('./снаружи.jpg', './y.png'));
  });
