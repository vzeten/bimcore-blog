...
// текст статьи не пишет; создать обложку нельзя — отказ словами, а не молчаливый выпуск без неё.
...
describe('обложка при публикации', () => {
  ...
  it('cover рядом, но внутри не PNG и не JPG либо расширение лжёт — публикация останавливается словами', async () => {
    const среда = репозиторий({[EN]: СТАТЬЯ(), [`${ПАПКА}/img-01.png`]: png(1200, 800), [`${ПАПКА}/cover.png`]: jpg(1200, 630)});

    const {code, data} = await запрос(среда, {path: EN});

    expect(code).toBe(409);
    expect(data).toEqual({error: НАСТРОЙКИ['ошибкиСервера']['обложкаНеГодна'], код: 'обложкаНеГодна'});
  });

  it('в тексте только анимация — годной картинки нет, и статья без обложки не выходит молча', async () => {
    ...
    expect(code).toBe(409);
    expect(data.код).toBe('обложкаНеСоздана');
    expect(снимокДиска(среда.repo)).toBe(до);
  });

  it('картинка не обработалась — отказ словами и ни одного нового файла', async () => {
    const среда = репозиторий({[EN]: СТАТЬЯ(), [`${ПАПКА}/img-01.png`]: png(1200, 800)});
    сБиблиотекой(ЛЕГЧЕ_PNG(() => {
    ...
    expect(code).toBe(409);
    expect(data.код).toBe('обложкаНеОбработалась');
    expect(снимокДиска(среда.repo)).toBe(до);
  });

  it('библиотеки обработки нет — отказ всей ручки словами, файлы не меняются', async () => {
    const среда = репозиторий({[EN]: СТАТЬЯ(), [`${ПАПКА}/img-01.png`]: png(1200, 800)});
    сБиблиотекой(null);
    const до = снимокДиска(среда.repo);

    await expect(запрос(среда, {path: EN}))
  ...
  it('поле обложки записано в несколько строк — отказ до создания файла: окно его строкой не перепишет', async () => {
    ...
    expect(code).toBe(409);
    expect(data.код).toBe('обложкаНеВписана');
    expect(снимокДиска(среда.repo)).toBe(до);
  });
