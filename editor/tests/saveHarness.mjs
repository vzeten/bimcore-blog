...
export const НАСТРОЙКИ = {
  хранение: {файлСостояния: '_state.json', папкаЧерновиков: '.drafts', папкаСпоров: '.drafts-споры', папкаСнимков: '.history', папкаСведения: '.сведение', черновикЖивётДней: 14, снимковНаВерсию: 50},
 ...
 *
 * 60 секунд с 2026-09-24: тесты аналитики изменений (ED-054) тоже гоняют настоящий git, общий прогон стал
 * дольше, и тесты публикации стали стабильно срываться на 30 секундах (по отдельности — зелёные).
 */
export const ЖДАТЬ_GIT = 60_000;
...
export async function сохранить(среда, {
  ...
  await saveRoute({
    ...
    фиксировать,
    // Опубликованной веткой здесь служит последний коммит песочницы: по нему сервер доказывает
    // базу у статьи, своих снимков ещё не имеющей, — ровно как у только что склонированной.
    publishedRef: () => 'HEAD',
