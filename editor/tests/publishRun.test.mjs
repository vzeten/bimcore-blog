...
describe('публикация от нажатия до сайта', () => {
  ...
  it('публиковать нечего, но в папке лежит лишний файл — он уходит в корзину, и человеку сказано что', async () => {
    const с = await среда();
    ...
    expect(исход.вид).toBe('остановка');
    expect(исход.ключ).toBe('лишнееУбрано');
    expect(исход.убрано).toEqual(['сирота.png']);
