 ...
 *
 * Порядок обхода задаёт, чьё слово победит при расхождении двух статей: сортировкой он один и тот
 * же на любой машине. Нечитаемый файл пропускается: без одной статьи список товаров всё равно
 * верен, а отказ на ровном месте закрыл бы вставку целиком.
 */
function названияСтатей(repo, settings) {
  const статьи = [];

  for (const rel of путиСтатей(repo, settings).sort()) {
    try {
      статьи.push({
