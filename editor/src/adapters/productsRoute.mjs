...
export async function productsRoute({req, res, url, repo, settings, тело, insideRepo, send}) {
  ...
  if (скачано.байты === undefined) {
    const причина = ошибки[скачано.причина].replace('{подробность}', скачано.подробность);
    send(res, 502, {error: ошибки['картинкаТовараНеДоехала'].replace('{причина}', причина)});
    return true;
  }
...
/**
 * Байты картинки товара; предел тот же, что у тела запроса. Отказ приходит с причиной — ключом
 * слов настроек и подробностью для отчёта: в статье и в её папке при этом ничего не меняется.
 */
async function скачать(адрес, settings) {
  try {
    const ответ = await fetch(адрес);
    if (!ответ.ok) return {причина: 'картинкаТовараОтказХранилища', подробность: String(ответ.status)};

    const байты = Buffer.from(await ответ.arrayBuffer());
    const предел = settings['сервер']['пределТелаМБ'];
    return байты.length > предел * 1024 * 1024 ? {причина: 'картинкаТовараПеревес', подробность: String(предел)} : {байты};
  } catch (error) {
    console.error(error);
    return {причина: 'картинкаТовараНетСвязи', подробность: error?.cause?.code ?? error?.message ?? String(error)};
  }
}
