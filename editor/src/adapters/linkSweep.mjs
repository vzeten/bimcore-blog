...
async function целиВерсий({git, repo, settings, версии, основы}) {
  ...
  for (const rel of версии) {
    ...
    // Адрес, живший на сайте ДО этой публикации, — всегда. Адреса с диска и из сегодняшней ветки
    // (её уже изменила сама публикация) — только если их не носит другая статья.
    const [после, до] = основы;
    const адрес = async (основа) => {
      const там = основа === undefined ? null : await showFile(git, основа, rel);
      return там === null ? '' : адресВерсии(rel, там, сайт);
    };
    const прежний = await адрес(до);
    if (прежний !== '') цели.адреса.add(прежний);
    const ещё = [...new Set([адресВерсии(rel, текст, сайт), await адрес(после)])].filter((свой) => свой !== '' && свой !== прежний);
    if (ещё.length > 0) {
      const кроме = new Set([rel, ...путиВерсийНаСайте(rel, settings).map((версия) => версия.путь)]);
      const занятые = await занятыеАдреса({git, repo, settings, основа: после ?? null, кроме, адреса: ещё});
      for (const свой of ещё) if (!занятые.has(свой)) цели.адреса.add(свой);
    }
    цели.файлы.add(rel);
