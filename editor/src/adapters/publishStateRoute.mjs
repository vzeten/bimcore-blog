 ...
 *
 * `лишние` — по записи на каждую версию, чей файл лежит у человека: имена файлов её папки, на
 * которые не ссылается ни один язык статьи. Окно показывает их рядом с языками ДО согласия
 * человека: публикация уберёт их в корзину и с сайта, и узнать об этом он обязан заранее.
 */
export async function состояниеНаСайте({repo, settings, git, rel}) {
  ...
  if (версии.length === 0) {
    return {
      path: rel,
      основаИзвестна: true,
      локали: [{локаль: null, путь: rel, ...своя(rel), наСайте: false, доступность: null, ссылок: 0, статьи: []}],
      лишние: лишниеОкна({repo, settings, версии: [rel]}),
    };
  }
  ...
  return {
    path: rel,
    основаИзвестна: лежит !== null,
    локали,
    лишние: лишниеОкна({repo, settings, версии: локали.filter((своя1) => своя1.файлЕсть).map((своя1) => своя1.путь)}),
  };
}

/**
 * Лишние файлы для окна: одна запись на версию, имена БЕЗ путей — человек читает имя файла, а не
 * место на диске. Фактов собрать не удалось — пустой перечень: уборки тогда не будет тоже.
 */
function лишниеОкна({repo, settings, версии}) {
