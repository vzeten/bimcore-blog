...
function сменить({payload, repo, settings, insideRepo, send, res}) {
  ...
  // Предел анимации проверяется здесь же, до первого шага операции: смена формата трогает
  // сразу два файла, и отказ обязан случиться раньше, чем на диске появится половина работы.
  const предел = settings['картинки']['пределГифМегабайт'];
  if (гифПеревесил(новое, bytes.length, предел)) {
    return send(res, 400, {error: текстОтказаГиф(ошибки['гифБольшеПредела'], предел)});
  }
  ...
  send(res, 200, {
    ...
    старыйОставлен,
    ...весКартинки(новое, bytes.length, settings['картинки']['максимумКилобайт']),
  });
}
