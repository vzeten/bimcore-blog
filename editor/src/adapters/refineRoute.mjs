...
async function применить({repo, settings, git, rel, dir, выбранные, ждали, send, res, последняяПравка}) {
  const ошибки = settings['ошибкиСервера'];
  // **Факты сайта спрашиваются заново и входят в отпечаток.** План мог сложиться, когда сайт был
  ...
  // Последняя сверка после всех `await`: статья, имена и байты медиа. Факты сайта берутся те же,
  // что уже вошли в снимок: сверяется диск, а не сайт, и второй вопрос к git открыл бы здесь новую
  // щель вместо того, чтобы её закрыть. Дальше до истории и файлов — только синхронные шаги.
  if (собратьСнимок(repo, rel, dir, снимок.наСайте).отпечаток !== снимок.отпечаток) {
    return send(res, 409, {error: ошибки['доработкаСнимокУстарел']});
  }
...
/**
 * Снимок одной версии: текст, байты используемых файлов папки, перечень имён (с одним уровнем
 * подпапок) и перечень имён, которые уже лежат на сайте.
 *
 * `наСайте` — `Set` местных имён файлов, найденных в опубликованной ветке, либо `null`: «спросить
 * не удалось». Оба ответа входят в отпечаток, и различить их обязательно — обработка картинок
 * ведёт себя по ним по-разному.
 */
export function собратьСнимок(repo, rel, dir, наСайте = null) {
  ...
  return {
    ...
    имена,
    наСайте,
    отпечаток: отпечаток(rel, байтыТекста, файлы, имена, наСайте),
  };
}
...
async function фактыСайта(git, repo, rel, dir) {
  ...
  // файла всё равно не наше дело, а соврать про сайт здесь опаснее, чем промолчать.
  return new Set(имена.filter((имя) => ответ.файлы.has(`${папка}/${имя}`)));
}
...
function отпечаток(rel, байтыТекста, файлы, имена, наСайте) {
  ...
  // Незнание про сайт и пустой перечень — разные состояния: в первом обработка не трогает ничего,
  // во втором обрабатывает всё. Один отпечаток на оба позволил бы применению сделать не то, что
  // показал план.
  кусок(Buffer.from(наСайте === null || наСайте === undefined
    ? 'сайт-неизвестен'
    : `сайт:${[...наСайте].sort().join('\n')}`, 'utf8'));
