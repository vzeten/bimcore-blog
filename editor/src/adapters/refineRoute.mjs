...
import {записатьОтпечатокПосле, сохранитьОригиналы} from './refineOriginals.mjs';
...
async function применить({repo, settings, git, rel, dir, выбранные, ждали, send, res, последняяПравка}) {
  ...
  // Оригиналы меняемых файлов обязаны лечь в хранилище раньше первой записи и совпасть по отпечатку:
  // не легли — записи не будет (решение владельца 2026-09-16: оригинал сохраняется до обработки).
  // Синхронно, как и всё после последней сверки.
  let папкаОригиналов = null;
  if (изменения.файлы.length > 0) {
    try {
      папкаОригиналов = сохранитьОригиналы({repo, settings, rel, снимок, изменения, когда: сейчас});
    } catch (ошибка) {
      console.error(ошибка);
      throw new ApiError(500, ошибки['оригиналНеСохранился']);
    }
  }
  ...
  if (папкаОригиналов !== null) {
    обслужить(() => записатьОтпечатокПосле({
      папка: папкаОригиналов,
      отпечаток: собратьСнимок(repo, rel, dir, снимок.наСайте).отпечаток,
    }), 'оригиналы');
  }
