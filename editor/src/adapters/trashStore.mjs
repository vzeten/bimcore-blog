// Корзина статей на диске: `editor/.trash/<запись>/` при МАТЕРИАЛАХ (хранилище редактора рядом с
// черновиками и историей), вне контента, сборки и git.
...
// продвижение в описи. Сама корзина не удаляет ничего и никогда: запись уходит только по прямому
// нажатию человека (`trashDrop.mjs`, решение владельца 2026-09-18).
// Опись и копии пишутся целиком: временный файл в служебной папке кода с `fsync` (тот же том, что
// и материалы, — иначе перенос откажет, оригиналы при этом целы), затем одна
...
// Папка корзины и папка записи перед любым чтением, стиранием или возвратом проверяются на
// подмену ссылкой или junction: чужую папку программа не читает и не стирает.
