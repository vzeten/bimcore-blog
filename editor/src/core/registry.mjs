...
/**
 * «Не доделано» (решение владельца 2026-09-21): хоть одного настроенного языка нет, он заглушка
 * или стоит «по ссылке». По правилу проекта «по ссылке» ставится только недоделанному, и этим
 * признаком ловятся и старые заглушки «available in Russian only», которых программа своим
 * образцом не узнаёт. Скрытие смотрится и в местной шапке (`скрыта`) — новые версии программа
 * создаёт скрытыми, и на сайт они ещё не выходили, — и на сайте (`поСсылке`).
 */
export function неДоделано(article, settings) {
  return Object.keys(settings['локали']).some((код) => {
    const версия = article.versions?.[код];
    if (!версия) return true;

    const признаки = признакиЛокали(версия);
    return признаки.заглушка || признаки.поСсылке || версия['скрыта'] === true;
  });
}

/**
 * Отбор, поиск и раздел действуют вместе. Оба отбора — про сайт, поэтому песочница (`наСайте`
 * ложно) в них не попадает никогда: публиковать и доделывать её для читателя нечего.
...
export function filterArticles(articles, filters, settings) {
  ...
  return articles.filter((article) => {
    ...
    if (filters.отбор === 'недоделано' && !(article.наСайте === true && неДоделано(article, settings))) return false;
