...
import {ЛЕНТА, ПОРЯДОК_МЕНЮ} from './menuOrder.mjs';
...
/**
 * Порядок, в котором раздел открывается сам, пока человек не щёлкнул по заголовку колонки
 * (решение владельца 2026-09-20).
 *
 * У ленты и у «Всех статей» сверху то, что правили последним: у блога бокового меню нет вовсе,
 * а в общем списке разделы перемешаны, и единственный понятный порядок там — свежесть правки.
 * У раздела документации сверху то, что стоит выше в боковом меню сайта: человек ищет статью
 * там же, где её видит читатель, а не по алфавиту и не по дате.
 *
 * `раздел` — узел дерева (`nodeId`) или пусто, если выбраны все статьи.
 */
export function порядокПоУмолчанию(раздел, settings) {
  const узел = String(раздел ?? '');
  const лента = узел === '' || узел === ЛЕНТА || узел.startsWith(`${ЛЕНТА}/`);

  return лента
    ? {колонка: settings['реестр']['сортировкаПоУмолчанию'], сторона: 'вниз'}
    : {колонка: ПОРЯДОК_МЕНЮ, сторона: 'вверх'};
}
