...
/**
 * Память о прошлом расчёте: слои, которые сравнивали в прошлый раз, и состояние на конец каждого. При
 * наборе история статьи не меняется — меняется только последний слой, набранный текст. Без памяти
 * каждая буква заново сравнивала бы всю статью со всеми прошлыми состояниями, и на статье в двадцать
 * с лишним тысяч знаков буква появлялась бы почти через секунду. Второго источника правды тут нет: в
 * памяти лежит итог тех же самых шагов, а слой с другим текстом или другими границами считается
 * заново вместе со всем, что за ним, — ответ на одни и те же слои от прошлых вызовов не зависит.
 */
let память: {служебное: Служебное; отпечатки: string[]; шаги: Состояние[]} | null = null;

/**
 * Слой для памяти — одной строкой из его ЗНАЧЕНИЙ: происхождение, границы правок, текст. Ссылку на сам
 * слой держать нельзя: память живёт между вызовами, а объект слоя и массив его границ вызывающий волен
 * изменить на месте — по ссылке такой слой сошёл бы за прежний и выдал бы старую окраску за свежую.
 * Отсутствие границ и пустой их список — разные случаи, поэтому у первого своё слово.
 */
function отпечаток(layer: Layer): string {
  const границы = layer.правки?.map((п) => `${п.fromA},${п.toA},${п.fromB},${п.toB}`).join(';');
  return `${layer.kind} ${границы ?? 'нет'} ${layer.text}`;
}
...
export function colorize(input: Layer[], служебное: Служебное = НЕТ_СЛУЖЕБНОГО): Colorized {
  ...
  const отпечатки = input.map(отпечаток);
  let общее = 0;
  while (общее < отпечатки.length && отпечатки[общее] === прежнее?.отпечатки[общее]) общее += 1;
  ...
  память = {служебное, отпечатки, шаги};
