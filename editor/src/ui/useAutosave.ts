...
export function useAutosave(
  ...
  // Что делать с ответом сервера. Автосохранение больше не одностороннее: сервер сверяет файл
  // с диском и может вернуть сведённый текст, новый отпечаток или отложенное расхождение.
  // Обработчик ставит окно — очередь создаётся раньше, чем он появляется.
  const ответСервера = useRef<(ответ: unknown) => void>(() => {});
  ...
  if (очередь.current === null) {
    ...
    }, undefined, (ответ) => ответСервера.current(ответ));
  }

  return {...очередь.current, послеЗаписи, ответСервера};
}
