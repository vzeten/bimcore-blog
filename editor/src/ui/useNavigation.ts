...
export function useNavigation(deps: {
  ...
  const closeArticle = async (): Promise<void> => {
    ...
    // Запись истории перестаёт указывать на статью: иначе обновление страницы после закрытия
    // (в том числе после удаления) пыталось бы открыть её снова и показывало бы отказ сервера
    // там, где человек сам ушёл в реестр (ED-039).
