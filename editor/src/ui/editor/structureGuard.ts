 ...
 * его открытие снизу; внутри контейнера у границ, рядом с пустыми или краем документа она — пустой
 * абзац, как и после жёсткого переноса. */
export function картаПоверхности(doc: Text, имена: ReadonlySet<string>): Область[] {
  ...
  for (let i = 0; i < строки.length; i += 1) {
    if (код[i]) continue;
    if (строки[i].trim() === '') {
      if (!жёсткие.includes(i - 1) && (текстом(i - 1) || текстом(i + 1) || закрытия.has(i - 1) || открытия.has(i + 1))) {
