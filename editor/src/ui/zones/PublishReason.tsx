 ...
 * имя блока или файла подставляются туда, где фраза их называет (`{текст}`, `{значение}`); номера
 * строки в тексте нет — к месту ведёт кнопка.
 */
export function словаНаходки(находка: PrepareFinding, settings: Settings): string {
  ...
  return фраза.split('{текст}').join(находка.текст || значение).split('{значение}').join(значение);
}
