...
export function AnalyticsScreen(props: {settings: Settings; articles: ArticleRow[]; onЗакрыть: () => void}) {
  ...
  const записи = (охват: string) => {
    ...
    const итог: {день: string; подпись: string; группа: string; язык: string | null}[] = [];
    for (const с of действия?.события ?? []) {
      ...
      if (подходит) итог.push({день: с.дата.slice(0, 10), подпись: `${вид}: ${название}`, группа: `${вид}: ${статьи.get(с.путь) ?? название}`, язык: с.локаль});
    }
    for (const р of действия?.ручные ?? []) {
      if (!страница || р.адрес === страница) итог.push({день: р.дата, подпись: `${п.ручное}: ${р.текст}`, группа: `${п.ручное}: ${р.текст}`, язык: null});
    }
    return итог;
  };
  ...
  const изменения = (охват: string, с: string, по: string): string[] => {
    const строки = new Map<string, Set<string>>();
    for (const з of записи(охват).filter((з) => з.день >= с && з.день <= по).sort((а, б) => а.день.localeCompare(б.день))) {
      const языки = строки.get(з.группа) ?? new Set<string>();
      if (з.язык) языки.add(з.язык.toUpperCase());
      строки.set(з.группа, языки);
    }
    const порядок = Object.keys(props.settings.локали).map((код) => код.toUpperCase());
    const поПорядку = (языки: Set<string>) => [...языки].sort((а, б) => порядок.indexOf(а) - порядок.indexOf(б)).join(', ');
    return [...строки].map(([подпись, языки]) => (языки.size ? `${подпись} — ${поПорядку(языки)}` : подпись));
  };
