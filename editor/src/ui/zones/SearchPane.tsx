...
export function SearchPane(props: {
  ...
  const открытьДни = (день: string) => props.onВид({охват: вид.охват, шаг: 'день', с: сдвигДня(день, -15), по: сдвигДня(день, 15)});
    ...
    <div className="search-pane">
      ...
      {стр?.по && !страница && (
        <div className="views-langs">
          <span className="views-label">{п.поЯзыкам}</span>
      ...
      {ряд && ряд.ряд.length > 0 && (
        <>
          <BarChart название={поиск ? п.показы : п.просмотры} столбики={столбики('показы')} коды={коды} цвета={цвета} цветные={цветные}
            метки={метки} подписьШага={подписьДня} частыеПодписи={вид.шаг !== 'день'} число={число} кратко={кратко} неполныйТекст={п.неполный} ещёТекст={п.ещёИзменений} onМетка={открытьДни} />
