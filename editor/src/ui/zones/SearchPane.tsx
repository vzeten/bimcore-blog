...
export function SearchPane(props: {
  ...
  const [выбор, setВыбор] = useState<{ключ: string; с: string; по: string; изменения: boolean} | null>(null);
  ...
  // Столбик — показатели периода (повторный щелчок снимает выбор); ▲ — ещё и список изменений, повторно — скрыть список.
  const выбрать = (номер: number, изменения = false) => {
    const начало = точки[номер].начало;
    if (выбран?.с === начало) return setВыбор(изменения ? {...выбран, изменения: !выбран.изменения} : null);
    ...
    setВыбор({ключ, с: начало, по, изменения});
  };
  const период = выбран
    ...
    <div className="search-pane">
      ...
      {ряд && ряд.ряд.length > 0 && (
        <>
          <BarChart название={поиск ? п.показы : п.просмотры} столбики={столбики('показы')} коды={коды} цвета={цвета} цветные={цветные}
            метки={метки} подписьШага={подписьДня} частыеПодписи={вид.шаг !== 'день'} число={число} кратко={кратко} неполныйТекст={п.неполный} сводкаТекст={п.сводкаИзменений} выбран={номерВыбран < 0 ? null : номерВыбран} onСтолбик={выбрать} onМетка={(номер) => выбрать(номер, true)} />
      ...
      {выбран && (
        <PeriodBox key={выбран.с} settings={props.settings} период={период} группы={props.изменения(вид.охват, выбран.с, выбран.по)}
          названия={props.названия} изменения={выбран.изменения} onСброс={() => setВыбор(null)} />
      )}
