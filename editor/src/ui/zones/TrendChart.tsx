...
/**
 * Круглая верхняя граница оси: 2, 4 или 10 на порядок, не меньше 2 — чтобы подпись середины оси была целым
 * числом (у границы 1 середина 0,5 округлялась до «1», проба 2026-09-24).
 */
export function граница(максимум: number): number {
  if (максимум <= 2) return 2;
  const порядок = 10 ** Math.floor(Math.log10(максимум));
  return [2, 4, 10].map((к) => к * порядок).find((в) => в >= максимум) ?? 20 * порядок;
}
