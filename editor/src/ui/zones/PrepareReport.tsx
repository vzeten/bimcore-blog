 ...
 *
 * У находки о блоке внутри текста поле называется подписью из настроек, а не именем свойства тега:
 * человек этого имени нигде не вводил и в свойствах блока не видел, искать ему по нему нечего.
 */
function место(находка: PrepareFinding): string {
  const поле = находка.свойство ?? находка.поле;
  const блок = находка.блок === undefined ? поле : [находка.блок, поле].filter(Boolean).join(': ');
  const строка = находка.строка === undefined ? undefined : `строка ${находка.строка}`;

  return [находка.путь, блок, строка].filter((часть) => typeof часть === 'string' && часть !== '').join(' · ');
}
