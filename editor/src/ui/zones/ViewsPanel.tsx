...
export function ViewsPanel(props: {
  ...
  const {всего, за30, прежние30, языки} = props.числа;
  // Пустые месяцы до первых просмотров не показываются: восемь нулей подряд ничего не говорят (владелец 2026-09-24).
  const первыйНепустой = props.числа.месяцы.findIndex((строка) => строка.просмотры > 0);
  const месяцы = первыйНепустой > 0 ? props.числа.месяцы.slice(первыйНепустой) : props.числа.месяцы;
