      ...
      <div className="topbar-actions">
        ...
        {props.article && !props.article.служебная && !props.просмотр && (
          props.удаление.предложение !== null && props.удаление.предложение.окно === окно ? (
            <>
              <span className="topbar-ask">
                {/* Куда уйдёт статья, одинаково: в корзину. Слова разные только про сайт. */}
                {label(props.удаление.предложение.причина === 'неОпубликована' ? 'удалениеНеБыла' : 'удалениеВКорзину', {
                  название: props.title, языки: props.удаление.предложение.языки.join(', '),
                })}
              </span>
              <button className="ghost ghost-danger" disabled={props.удаление.идёт} onClick={() => void props.удаление.удалить()}>
                {props.удаление.идёт ? п.удалениеИдёт : п.вКорзину}
              </button>
