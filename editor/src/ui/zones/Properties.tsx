      ...
      {open && (
        <div className="props-grid">
              ...
              {(field.kind === 'заперто' || НЕПРАВИМЫЕ.includes(field.key)) && (
                <span className="props-locked">
                  {props.settings.полеЗаперто[
                    field.kind === 'заперто' ? (field.причина ?? 'неПрочитано') : 'неПравитсяЗдесь'
                  ]}
                </span>
              )}
          ...
          <label>
            ...
            <select
              className="props-режим"
              value={режим}
              disabled={props.толькоЧтение === true}
              onChange={(event) => выбратьРежим(event.target.value)}
            >
              {(РЕЖИМЫ as string[]).map((значение) => (
                <option key={значение} value={значение}>{props.settings.доступность[значение]}</option>
              ))}
            </select>
          </label>
        </div>
