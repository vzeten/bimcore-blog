...
export function App() {
  ...
  const {подстановка, вЧерновик, правка, правитьПоля, поставитьДату, поставитьОбложку, отметить, положитьПару} = useWindowText({
  ...
  return (
    <div className={colors ? 'app' : 'app mono'}>
      <TopBar
        ...
        onColors={setColors}
        onSave={save}
        // Дату записи блога и обложку ставит публикация: строка ложится в шапку окна и уезжает на диск
        // тем же сохранением, что и любая правка человека (`core/publishDate.mjs`).
