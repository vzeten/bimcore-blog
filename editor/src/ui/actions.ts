...
export interface Effects {
  ...
  ok: (ответ?: {
    ...
    /**
     * Что стало с видимостью остальных языковых версий. Она признак статьи целиком, поэтому
     * сохранение переключает их вместе с открытой; человек обязан узнать поимённо, какие версии
     * поменялись, а какие остались прежними из-за сбоя.
     */
    видимость?: {переключены: string[]; неПереключены: string[]};
    устарел?: boolean;
  }) => void;
