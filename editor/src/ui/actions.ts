...
export interface Effects {
   ...
   */
  ok: (ответ?: {отпечаток?: string; предупреждения?: string[]; устарел?: boolean}) => void;
...
export async function setVisibility(
  ...
  try {
    ...
    // Файлы записаны, но окно за это время могло уехать в другую статью: применять к ней
    // видимость прошлой нельзя.
    if (актуально()) effects.ok();
  } catch (error) {
