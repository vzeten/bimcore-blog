...
// возможен только явным переключателем испытательного режима. Корень не годится — программа не
// стартует и говорит причину словами, вместо тихой работы не над теми файлами. Тем же заходом
// экземпляр опознаётся целиком: род, версия КОДА и порт, причём `4780` принадлежит обычному
// принятому окну, а другому экземпляру номер называют явно (SPEC 4.3, 7.1.6).
let ИСТОЧНИК;
let ОПОЗНАНИЕ;
try {
...
const vite = await createVite({
  root: EDITOR_DIR,
  appType: 'spa',
  // Порт горячей перезагрузки — свой у каждого экземпляра: общий у Vite один, и второй запуск на нём ругается.
  server: {middlewareMode: true, hmr: {port: 20000 + (PORT % 10000)}},
...
const сервер = http
  ...
  .listen(PORT, () => {
    // Экземпляр обязан назвать себя сам: род окна, версию КОДА, папку материалов и адрес, а следом —
    // где лежит незаписанная работа. Без этих строк окно на чужом дереве выглядело как обычное.
    for (const строка of [
...
// Порт занят — не повод падать простынёй стека: человеку говорится, что занято, и чужой сервер
// при этом не трогается. Своё окно ярлык на нём не открывает: опознание не совпадёт.
