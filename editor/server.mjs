...
// возможен только явным переключателем испытательного режима, и тогда экземпляр называет себя
// испытательным при запуске. Корень не годится — программа не стартует и говорит причину словами,
// вместо тихой работы не над теми файлами. Правило и его отказы — в ядре и в своём адаптере (SPEC 4.3).
let ИСТОЧНИК;
try {
  ИСТОЧНИК = материалыВладельца(readSettings(), process.env[ПЕРЕКЛЮЧАТЕЛЬ]);
} catch (error) {
...
http
  ...
  .listen(PORT, () => {
    // Экземпляр обязан назвать себя сам: род окна, версию КОДА, фактическую папку материалов и
    // адрес. Пока этих строк не было, окно на чужом дереве выглядело точно так же, как обычное.
    for (const строка of строкиЗапуска({
      settings: readSettings(),
      кодDir: EDITOR_DIR,
      версия: версияЭтогоКода,
      корень: REPO,
